...
{
#ifdef VM
  hash_init (& thread_current()->vm, page_hash, page_less, NULL);
#endif  //VM
...
#ifdef VM
    hash_destroy (& cur->vm, free_hash_elem);
#endif // VM
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      // TODO allocate vm_entry (use malloc)
      // TODO initialize vm_entry (offset, size, zero byte pad)
      // TODO insert vm_entry into vm hash table
      // TODO delete allocating and mapping physical part
...
setup_stack (void **esp) 
{
  ...
  // We don't delete this page allocation because there is no where to load from (page fault is only for things in memory to be loaded from disk)
  // user program virtual pages are one to one with vm_entry (may or not be in memory)


  kpage = palloc_get_page (PAL_USER | PAL_ZERO);
  if (kpage != NULL)
    {
      uint8_t* VPN = ((uint8_t *) PHYS_BASE) - PGSIZE;
      //success = install_page (((uint8_t *) PHYS_BASE) - PGSIZE, kpage, true);
        success = install_page (VPN, kpage, true);
      if (success) {
          *esp = PHYS_BASE;
#ifdef VM
          // TODO:
        // allocate vm entry of 4kb stack (must use malloc so element doesn't die after leaving scope of this function)
        // initialize field
